...
//  tm_ios_library
//
//  Created by Tatsunori on 2021/02/23.
//
...
class TestHelper: NSObject {
    static func generateVideoFrames(from phAsset: PHAsset, numberOfFrames: Int = 9, completion: @escaping ([CGImage]) -> Void) {
        DispatchQueue.global(qos: .userInitiated).async {
            ...
            multiTask.enter()
            TestHelper.requestAVAsset(asset: phAsset, complete: { avAsset in
                asset = avAsset
                multiTask.leave()
            })
            multiTask.wait()
...
//        manager.cancelImageRequest(pId)
    ...
    static func getAssets(allowMediaTypes: [TestMediaType]) -> [PHAsset] {
